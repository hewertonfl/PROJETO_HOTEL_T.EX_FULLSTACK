-- MySQL Script generated by MySQL Workbench
-- ter 28 fev 2023 08:34:24
-- Model: New Model    Version: 1.0
-- MySQL Workbench Forward Engineering

SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0;
SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0;
SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION';

-- -----------------------------------------------------
-- Schema hotel_recanto
-- -----------------------------------------------------

-- -----------------------------------------------------
-- Schema hotel_recanto
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `hotel_recanto` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci ;
USE `hotel_recanto` ;

-- -----------------------------------------------------
-- Table `hotel_recanto`.`usuario`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `hotel_recanto`.`usuario` (
  `id_usuario` INT NOT NULL AUTO_INCREMENT,
  `nome_usuario` VARCHAR(45) NOT NULL,
  `sobrenome_usuario` VARCHAR(45) NOT NULL,
  `email_usuario` VARCHAR(80) NOT NULL,
  `nivel_usuario` TINYINT(10) NOT NULL,
  `status_usuario` VARCHAR(10) NOT NULL,
  `senha_usuario` VARCHAR(255) NOT NULL,
  PRIMARY KEY (`id_usuario`),
  UNIQUE INDEX `email_usuario_UNIQUE` (`email_usuario` ASC) VISIBLE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_0900_ai_ci;


-- -----------------------------------------------------
-- Table `hotel_recanto`.`acomodacao`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `hotel_recanto`.`acomodacao` (
  `id_acomodacao` INT NOT NULL AUTO_INCREMENT,
  `numero_acomodacao` INT UNSIGNED NOT NULL,
  `tipo_acomodacao` VARCHAR(80) NOT NULL,
  `descricao_acomodacao` LONGTEXT NOT NULL,
  `preco_acomodacao` DECIMAL(10,2) UNSIGNED NOT NULL,
  `imagem_acomodacao` VARCHAR(255) NOT NULL,
  `status_acomodacao` VARCHAR(10) NOT NULL,
  PRIMARY KEY (`id_acomodacao`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_0900_ai_ci;


-- -----------------------------------------------------
-- Table `hotel_recanto`.`cupom`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `hotel_recanto`.`cupom` (
  `id_cupom` INT NOT NULL AUTO_INCREMENT,
  `codigo_cupom` VARCHAR(10) NOT NULL,
  `data_cupom` DATETIME NOT NULL,
  `validade_cupom` DATETIME NOT NULL,
  `desconto_cupom` INT(2) UNSIGNED NOT NULL,
  PRIMARY KEY (`id_cupom`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_0900_ai_ci;


-- -----------------------------------------------------
-- Table `hotel_recanto`.`avaliacao`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `hotel_recanto`.`avaliacao` (
  `id_avaliacao` INT NOT NULL AUTO_INCREMENT,
  `data_avaliacao` DATETIME NOT NULL,
  `pontuacao_avaliacao` TINYINT(5) NOT NULL,
  `descricao_avaliacao` LONGTEXT NOT NULL,
  `status_avaliacao` VARCHAR(20) NOT NULL,
  PRIMARY KEY (`id_avaliacao`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_0900_ai_ci;


-- -----------------------------------------------------
-- Table `hotel_recanto`.`reserva`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `hotel_recanto`.`reserva` (
  `id_reserva` INT NOT NULL AUTO_INCREMENT,
  `checkin_reserva` DATE NOT NULL,
  `checkout_reserva` DATE NOT NULL,
  `qtd_pessoas_reserva` INT(10) NOT NULL,
  `total_reserva` DECIMAL(10,2) NOT NULL,
  `total_desconto_reserva` DECIMAL(10,2) NULL,
  `data_reserva` DATE NOT NULL,
  `confirmacao_reserva` TINYINT NOT NULL,
  `data_confirmacao_reserva` DATE NOT NULL,
  `acomodacao_id_acomodacao` INT NOT NULL,
  `cupom_id_cupom` INT NOT NULL,
  `avaliacao_id_avaliacao` INT NOT NULL,
  `usuario_id_usuario` INT NOT NULL,
  PRIMARY KEY (`id_reserva`),
  INDEX `fk_reserva_acomodacao_idx` (`acomodacao_id_acomodacao` ASC) VISIBLE,
  INDEX `fk_reserva_cupom1_idx` (`cupom_id_cupom` ASC) VISIBLE,
  INDEX `fk_reserva_avaliacao1_idx` (`avaliacao_id_avaliacao` ASC) VISIBLE,
  INDEX `fk_reserva_usuario1_idx` (`usuario_id_usuario` ASC) VISIBLE,
  CONSTRAINT `fk_reserva_acomodacao`
    FOREIGN KEY (`acomodacao_id_acomodacao`)
    REFERENCES `hotel_recanto`.`acomodacao` (`id_acomodacao`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_reserva_cupom1`
    FOREIGN KEY (`cupom_id_cupom`)
    REFERENCES `hotel_recanto`.`cupom` (`id_cupom`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_reserva_avaliacao1`
    FOREIGN KEY (`avaliacao_id_avaliacao`)
    REFERENCES `hotel_recanto`.`avaliacao` (`id_avaliacao`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_reserva_usuario1`
    FOREIGN KEY (`usuario_id_usuario`)
    REFERENCES `hotel_recanto`.`usuario` (`id_usuario`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_0900_ai_ci;


-- -----------------------------------------------------
-- Table `hotel_recanto`.`adicionais`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `hotel_recanto`.`adicionais` (
  `id_adicionais` INT NOT NULL AUTO_INCREMENT,
  `item_adicionais` VARCHAR(45) NOT NULL,
  `descricao_adicionais` LONGTEXT NOT NULL,
  `valor_adicionais` DECIMAL(10,2) UNSIGNED NOT NULL,
  `imagem_adicionais` VARCHAR(255) NOT NULL,
  PRIMARY KEY (`id_adicionais`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_0900_ai_ci;


-- -----------------------------------------------------
-- Table `hotel_recanto`.`consumo`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `hotel_recanto`.`consumo` (
  `id_consumo` INT NOT NULL AUTO_INCREMENT,
  `item_consumo` VARCHAR(45) NOT NULL,
  `preco_consumo` DECIMAL(10,2) UNSIGNED NOT NULL,
  PRIMARY KEY (`id_consumo`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_0900_ai_ci;


-- -----------------------------------------------------
-- Table `hotel_recanto`.`adicionais_reserva`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `hotel_recanto`.`adicionais_reserva` (
  `adicionais_id_adicionais` INT NOT NULL,
  `reserva_id_reserva` INT NOT NULL,
  PRIMARY KEY (`adicionais_id_adicionais`, `reserva_id_reserva`),
  INDEX `fk_adicionais_has_reserva_reserva1_idx` (`reserva_id_reserva` ASC) VISIBLE,
  INDEX `fk_adicionais_has_reserva_adicionais1_idx` (`adicionais_id_adicionais` ASC) VISIBLE,
  CONSTRAINT `fk_adicionais_has_reserva_adicionais1`
    FOREIGN KEY (`adicionais_id_adicionais`)
    REFERENCES `hotel_recanto`.`adicionais` (`id_adicionais`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_adicionais_has_reserva_reserva1`
    FOREIGN KEY (`reserva_id_reserva`)
    REFERENCES `hotel_recanto`.`reserva` (`id_reserva`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_0900_ai_ci;


-- -----------------------------------------------------
-- Table `hotel_recanto`.`reserva_consumo`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `hotel_recanto`.`reserva_consumo` (
  `reserva_id_reserva` INT NOT NULL,
  `consumo_id_consumo` INT NOT NULL,
  `local_reserva_consumo` VARCHAR(45) NOT NULL,
  `data_reserva_consumo` DATE NOT NULL,
  `qtd_item_reserva_consumo` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`reserva_id_reserva`, `consumo_id_consumo`),
  INDEX `fk_reserva_has_consumo_consumo1_idx` (`consumo_id_consumo` ASC) VISIBLE,
  INDEX `fk_reserva_has_consumo_reserva1_idx` (`reserva_id_reserva` ASC) VISIBLE,
  CONSTRAINT `fk_reserva_has_consumo_reserva1`
    FOREIGN KEY (`reserva_id_reserva`)
    REFERENCES `hotel_recanto`.`reserva` (`id_reserva`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_reserva_has_consumo_consumo1`
    FOREIGN KEY (`consumo_id_consumo`)
    REFERENCES `hotel_recanto`.`consumo` (`id_consumo`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_0900_ai_ci;


SET SQL_MODE=@OLD_SQL_MODE;
SET FOREIGN_KEY_CHECKS=@OLD_FOREIGN_KEY_CHECKS;
SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS;
